      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <div className="bg-slate-800 px-4 py-4 md:p-6 rounded-2xl shadow-xl border border-slate-700 flex justify-between items-center gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <img
              ...
              alt="Edgar"
              className="h-10 w-auto md:h-16 flex-shrink-0 hover:scale-105 transition-transform"
            />
            <div className="min-w-0">
              <h1 className="text-base md:text-xl font-black text-white tracking-tighter leading-tight">
                EDGE<span className="text-[#2b998e]">LOCALIST</span>
              </h1>
              <p className="text-[10px] text-slate-400 font-mono truncate max-w-[180px] md:max-w-none">
                {status || 'Ready'}
              </p>
            </div>
          </div>
          <button
            onClick={onLogout}
            className="flex-shrink-0 flex items-center gap-2 px-3 py-2 md:px-4 bg-slate-700 hover:bg-red-900/40 active:bg-red-900/60 rounded-xl text-sm transition-all"
            aria-label="Logout"
          >
            <LogOut size={16} />
        ...
        {error && (
          <div className="bg-red-900/40 border border-red-700 text-red-300 px-4 py-3 rounded-xl text-sm flex justify-between items-start gap-3">
            ...
            <button
              onClick={() => setError('')}
              className="text-red-400 hover:text-white flex-shrink-0 p-1 -mt-0.5"
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden">
          ...
          >
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400">
              Database Geocoder
              {geoCount.total > 0 && (
                <span className="ml-2 text-[#2b998e]">({geoCount.current}/{geoCount.total})</span>
              )}
            </h3>
          ...
          <div className={`px-4 pb-4 md:px-6 md:pb-6 space-y-4 ${geocoderOpen ? 'block' : 'hidden md:block'}`}>
            ...
            {!isLoading && !isPaused ? (
              <button
                onClick={() => geocodeAllContacts(0)}
                className="w-full py-4 bg-white text-slate-900 rounded-xl font-black text-sm uppercase tracking-widest hover:bg-slate-200 active:bg-slate-300 flex justify-center items-center gap-2 transition-all"
              >
        ...
        <div className="bg-slate-800 p-4 md:p-6 rounded-2xl border border-slate-700 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-3 text-slate-400 hover:text-[#2b998e] active:text-[#2b998e] transition-colors"
          ...
          {recentSearches.length > 0 && (
            <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
              {recentSearches.map((s, i) => (
                ...
                >
                  <MapPin size={11} className="text-[#2b998e]" />
                  {s.address.length > 28 ? s.address.slice(0, 28) + '…' : s.address}
                </button>
              ))}
            </div>
          )}
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-[#2b998e] py-4 rounded-xl font-bold text-base md:text-lg hover:bg-[#238278] active:bg-[#1d6e65] transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 overflow-hidden shadow-2xl">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <h3 className="font-bold text-base md:text-lg">
                  Results <span className="text-[#2b998e]">{filteredContacts.length}</span>
                </h3>
                <div className="relative">
                  ...
                  <select
                    ...
                    className="bg-slate-900 border border-slate-700 rounded-lg pl-7 pr-3 py-2 text-sm text-[#2b998e] outline-none focus:ring-2 focus:ring-[#2b998e] appearance-none"
                  >
              ...
              <input
                ...
                className="w-full sm:w-48 bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-base sm:text-sm outline-none focus:ring-2 focus:ring-[#2b998e]"
          ...
          <div
            className="divide-y divide-slate-700 overflow-y-auto"
          ...
          >
            ...
            {filteredContacts
              .filter(c => `${c.firstName} ${c.lastName}`.toLowerCase().includes(nameFilter.toLowerCase()))
              .map(contact => (
                <div key={contact.id} className="p-4 md:p-5 hover:bg-slate-700/40 active:bg-slate-700/60 transition-all">
                  <div className="flex justify-between items-start mb-3 gap-3">
                    <div className="min-w-0">
                      <h4 className="font-bold text-white text-base md:text-lg leading-tight truncate">
                    ...
                    <div className="text-right flex-shrink-0">
                      <span className="block text-2xl font-black text-[#2b998e] leading-none">
                        {contact.distance}
                      </span>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <a
                      ...
                      className="flex items-center justify-center gap-2 py-3 bg-slate-900 rounded-xl text-xs font-bold uppercase tracking-wide hover:bg-blue-600 active:bg-blue-700 transition-colors"
                    >
                      <Phone size={14} /> Call
                    </a>
                    <button className="flex items-center justify-center gap-2 py-3 bg-slate-900 rounded-xl text-xs font-bold uppercase tracking-wide hover:bg-teal-600 active:bg-teal-700 transition-colors">
                      <Phone size={14} /> GHL Call
                    </button>
                    <a
                      href={`sms:${contact.phone}`}
                      className="flex items-center justify-center gap-2 py-3 bg-slate-900 rounded-xl text-xs font-bold uppercase tracking-wide hover:bg-indigo-600 active:bg-indigo-700 transition-colors"
                    >
                      <MessageSquare size={14} /> SMS
                    </a>
                    <button className="flex items-center justify-center gap-2 py-3 bg-slate-900 rounded-xl text-xs font-bold uppercase tracking-wide hover:bg-purple-600 active:bg-purple-700 transition-colors">
